    ...
    for i, data in enumerate(tqdm(dataloader)):
        with torch.no_grad():
            outputs = model(
                ...
                # entity_ids=data["entity_ids"].to(device)  # entity ids 추가
            )
        logits = outputs[0]
...
def load_test_dataset(dataset_dir, tokenizer):
    ...
    # tokenized_test = tokenized_testset(test_dataset, tokenizer)
    return test_dataset["id"], tokenized_test, test_label

exp_name = 'w400lr2e5-punct2-obj'
def main(args):
    """
    ...
    if args.ensemble == False:
        ...
        # model = MyRobertaForSequenceClassification.from_pretrained(MODEL_NAME)
...
#         pred_answer = num_to_label(pred_answer)
    ...
    output.to_csv(
        f"./prediction/{exp_name}-soft.csv", index=False
...
if __name__ == "__main__":
    ...
    # model dir
    parser.add_argument("--model_dir", type=str, default=f"./best_model/{exp_name}")
